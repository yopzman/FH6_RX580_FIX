...
static void ParseStreamAndLogShaders(const D3D12_PIPELINE_STATE_STREAM_DESC* pDesc) {
    ...
    while (ptr < end) {
        ...
        if (type == 2) { // VS
            const auto* bytecode = reinterpret_cast<const D3D12_SHADER_BYTECODE*>(ptr + sizeof(void*));
            ...
            case 2: case 3: case 4: case 5: case 6: case 8: case 13: case 20: size = 24; break;
            case 7: size = 48; break;
            case 9: size = 328; break;
            case 10: case 14: case 15: case 17: case 18: case 19: size = 16; break;
            case 11: size = 56; break;
            case 12: size = 64; break;
            case 16: size = 48; break;
            case 21: size = 32; break;
            default:
                return;
            }
            ptr += size;
...
static HRESULT STDMETHODCALLTYPE HookedCreateComputePipelineState(
    ...
    if (pDesc) {
        ...
        char buf[256] {};
        std::snprintf(buf, sizeof(buf),
            "CreateComputePipelineState: CS bytecode size=%zu, SM minor=%u, highSM=%s",
    ...
    if (SUCCEEDED(hr)) {
        char buf[256] {};
        std::snprintf(buf, sizeof(buf),
            "CreateComputePipelineState: original pipeline created successfully, HRESULT 0x%08lX",
    ...
    if (pDesc && ppPipelineState) {
            ...
            if (SUCCEEDED(hrFallback)) {
                ...
                char errBuf[256] {};
                std::snprintf(errBuf, sizeof(errBuf), "CreateComputePipelineState: Fallback ALSO failed! HRESULT 0x%08lX", static_cast<unsigned long>(hrFallback));
                Log(errBuf);
            }
        }
    return hr;
...
static HRESULT STDMETHODCALLTYPE HookedCreatePipelineState(
    ...
    if (SUCCEEDED(hr)) {
        char buf[256] {};
        std::snprintf(buf, sizeof(buf),
            "CreatePipelineState (Stream): original pipeline created successfully, HRESULT 0x%08lX",
            static_cast<unsigned long>(hr));
        Log(buf);
        return hr;
    }
    ...
    if (pDesc && ppPipelineState) {
        ...
        if (SUCCEEDED(hrFallback)) {
            ...
            char errBuf[256] {};
            std::snprintf(errBuf, sizeof(errBuf), "CreatePipelineState (Stream): Fallback ALSO failed! HRESULT 0x%08lX", static_cast<unsigned long>(hrFallback));
            Log(errBuf);
        }
    }
    return hr;
...
static D3D12_PIPELINE_STATE_STREAM_DESC MutateStreamCS(
    ...
    while (ptr < end) {
        ...
        if (type == 8) { // CS (Compute Shader)
            auto* bytecode = reinterpret_cast<D3D12_SHADER_BYTECODE*>(ptr + sizeof(void*));
